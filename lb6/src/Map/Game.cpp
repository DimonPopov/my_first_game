...
Game::Game(){
	save = std::make_unique<SaveClass>();
}
...
void Game::menuWindow(){
    ...
    sf::Font font;
    font.loadFromFile("./texture/2.ttf");
...
void Game::newGame(){
	...
	if(!map){
		...
		std::cout << "Новая карта\n";
	}
	if(!player){
		player = std::make_unique<Player>();
		std::cout << "Новый игрок\n";
	}

	if(!enemy1){
		enemy1 = std::make_shared<Enemy1>();
		enemy2 = std::make_shared<Enemy2>();
		enemy3 = std::make_shared<Enemy3>();
	}
	...
	sf::Font font;
	font.loadFromFile("./texture/1.ttf");
		...
		while (window.pollEvent(event)){
			...
			if (event.type == sf::Event::KeyPressed){
				...
				if(event.key.code == sf::Keyboard::L){
					save->saving(player, map, enemy1, enemy2, enemy3);
					this->endGame();
				}
		...
		for(size_t i = 0; i < map->getWidth(); i++){
			for(size_t j = 0; j < map->getHeight(); j++){
				...
				if(i == player->myPositionX() && j == player->myPositionY()){
					...
					if(CellValue == CellTypes::EXIT && player->getCoins() >= 4){
...
void Game::loadGame(){
	save->loading(player, map, enemy1, enemy2, enemy3);
	this->newGame();
}
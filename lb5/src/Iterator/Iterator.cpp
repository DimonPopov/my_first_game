...
Iterator::Iterator(int whoAreYou){
    ...
    }else if(whoAreYou == PlayerOrEnemy::IamENEMY){
        srand(time(nullptr));
        while(1){
            m_positionX = rand() % stc->m_width;
            m_positionY = rand() % stc->m_height;
            if(stc->m_arrField[m_positionX][m_positionY].getValue() == CellTypes::OPEN && stc->m_arrField[m_positionX][m_positionY].isEnemy != 1 && stc->m_arrField[m_positionX][m_positionY].getValue() != CellTypes::START){
                stc->m_arrField[m_positionX][m_positionY].isEnemy = 1;
                break;
            }
        }
    }
